...
int main(int argc, char**argv) {
    ...
    try {
        ...
        if (size_stream % challenge::SIZE_PACKET) {
            ...
            cout << size_stream << " is not a multiple of "
                    << challenge::SIZE_PACKET << endl;
            return -1;
        } else {
            cout << " The stream contains ["
                    << size_stream / challenge::SIZE_PACKET << "] packets"
                    << endl;
        }
        ...
        cout << e.what() << endl;
        cout << " on byte at position 0x" << hex << e.get_position() << dec
                << " " << e.get_position() << endl;
        return -1;
    } catch (exception & e) {
        cout << e.what() << endl;
