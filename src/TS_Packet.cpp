...
namespace challenge {
    ...
    int TS_Packet::get_transport_private_data_length() {
        int offset = get_PCR_flag() * 6 + get_OPCR_flag() * 6
    ...
    int TS_Packet::get_adaptation_field_extension_length() {
        int offset = get_PCR_flag() * 6 + get_OPCR_flag() * 6
                + get_splicing_point_flag()
                + get_transport_private_data_flag()
                        * (1 + get_transport_private_data_length());
    ...
    int TS_Packet::get_below_header_offset() {
        ...
        if (afc == 3) {
            ...
            if (this->is_carrying_PES_packet()) {
                ...
                try {
                    PES_packet pes_packet(buffer + (4 + 1 + len_af_payload),
                            SIZE_PACKET - (4 + 1 + len_af_payload));
                    return 1 + len_af_payload
                            + pes_packet.get_PES_not_payload_length();
    ...
    std::ostream& operator<<(std::ostream &output, TS_Packet &o) {
        ...
        for (size_t i = 0; i < SIZE_PACKET; i++) {
            ...
            output << std::hex << std::uppercase
                    << static_cast<int>(o.buffer[i]) << std::dec << " ";
        }
        output << std::endl;
